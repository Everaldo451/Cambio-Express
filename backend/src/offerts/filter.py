...
class OffertFilter(django_filters.FilterSet):
    ...
    percent_min = django_filters.CharFilter(field_name="percent", lookup_expr="gte")
    percent_max = django_filters.CharFilter(field_name="percent", lookup_expr="lte")

    class Meta:
        model = Offert
        fields = ["min_value", "code", "index_variable", "percent"]